...
(function() {
  'use strict';
  var debugmode, i, isMPS, mps, mpscall, mpsinstance, mpsopts, _base;
  ...
  mps._ext = {
    ...
    'nowrite': 0,
    '_insertedads': [],
    '_jq': typeof (_base = typeof jQuery === 'function') === "function" ? _base({
      1: 0
    }) : void 0
  };
  ...
  mpsopts.deriveparams = mpsopts.deriveparams || {
    1: 'cat1',
    2: 'cat2',
    3: 'cat3',
    4: 'cat4',
  ...
  if (!mps._loadset) {
    ...
    mps._queue.render = function(type, slot, loadset) {
        ...
        default:
          ...
          while (i < mps._queue[type].length) {
            if (typeof (loadset === 'number')) {
              mps._queue[type][i].call(this, slot, loadset);
            } else {
              mps._queue[type][i].call(this, slot);
            }
            i++;
          }
          break;
      }
      return mps._queue.clear = function(type) {
        var _results;
        if (type) {
          mps._debug('[mps/Loader] MPS QUEUE: (clear ' + type + ')');
          if (mps._queue[type]) {
            return mps._queue[type] = [];
          } else {
            return mps._debug('[mps/Loader] MPS QUEUE: (clear ' + type + ') is not a valid queue.');
          }
        } else {
